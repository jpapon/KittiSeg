...
def evaluation(hyp, images, labels, decoded_logits, losses, global_step):
    ...
    neg_labels = tf.to_int32(tf.logical_or (labels[:, 0], labels[:, 1]))
    pos_labels = tf.logical_or (labels[:, 2], labels[:, 3])
    pos_labels = tf.to_int32(tf.logical_or (labels[:, 4],pos_labels))

    negativ = tf.to_int32(tf.logical_or(tf.equal(pred, 0),tf.equal(pred, 1)))
    tn = tf.reduce_sum(negativ*neg_labels)
    fn = tf.reduce_sum(negativ*pos_labels)

    positive = tf.logical_or(tf.equal(pred, 2),tf.equal(pred, 3))
    positive = tf.to_int32(tf.logical_or(positive,tf.equal(pred, 4)))
    tp = tf.reduce_sum(positive*pos_labels)
    fp = tf.reduce_sum(positive*neg_labels)
