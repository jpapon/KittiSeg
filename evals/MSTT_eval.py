...
def evaluate(hypes, sess, image_pl, inf_out):
    ...
    with open(data_file) as file:
        for i, datum in enumerate(file):
                ...
                #tube_im = output[0][:, 1].reshape(shape[0], shape[1])

                tube_im = output[0][:, 2:5].reshape(shape[0], shape[1], 3)
                tube_im = tf.reduce_max(tube_im, axis = 2)
                tube_im = sess.run (tube_im)

                if hypes['jitter']['fix_shape']:
                    gt_shape = gt_image.shape
                    tube_im = tube_im[offset_x:offset_x+gt_shape[0],
                                          offset_y:offset_y+gt_shape[1]]

                if True:
                    # Saving RB Plot
                    ov_image = seg.make_overlay(image, tube_im)
                    ...
                    label_im = tf.argmax(label_im, axis = 2)
                    label_im = sess.run(label_im)
                    label_im = utils.overlay_segmentation(image, label_im, label_colors)
                    image_list.append((name2, label_im))

                FN, FP, posNum, negNum = eval_image(hypes, gt_image, tube_im)
