...
def eval_image(hypes, gt_image, cnn_image):
    """."""
    cnn_image = np.nan_to_num (cnn_image)

    thresh = np.array(range(0, 256))/255.0
    #Grass, Dirt
    road_gt = np.logical_or (gt_image[:, :, 1] == 255, gt_image[:, :, 2] == 32)
    #Pavement
    road_gt = np.logical_or (road_gt, gt_image[:, :, 2] == 128)

    valid_gt = np.logical_or(gt_image[:, :, 0] > 0,gt_image[:, :, 1] > 0)
...
def evaluate(hypes, sess, image_pl, inf_out):
    ...
    for key in hypes['data'].keys():
      if ('_color' in key):
        ...
        label_colors[color[0]] = (color[1],color[2],color[3],175)
    label_colors[0] = (0,0,0,128)
    ...
    with open(data_file) as file:
        for i, datum in enumerate(file):
                ...
                class_of_interest = 1
                class_score_im = output[0][:, class_of_interest].reshape(shape[0], shape[1])
                ...
                if True:
                    conf_img = np.max(output[0].reshape(shape[0], shape[1],num_classes),axis = 2)

                    # Saving RB Plot
                    ov_image = seg.make_overlay(image, conf_img)
                    name = os.path.basename(image_file)
                    name_conf = name.split('.')[0] + '_confidence.png'
                    image_list.append((name_conf, ov_image))

                    name2 = name.split('.')[0] + '_labels.png'

                    num_classes = hypes['arch']['num_classes']
                    label_im = output[0].reshape(shape[0], shape[1], num_classes)

                    label_im = np.argmax(label_im, axis = 2)

                    label_im = utils.overlay_segmentation(image, label_im, label_colors)
                    image_list.append((name2, label_im))

                FN, FP, posNum, negNum = eval_image(hypes, gt_image, class_score_im)
