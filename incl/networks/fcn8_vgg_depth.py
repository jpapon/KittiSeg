...
class FCN8VGG:
    ...
    def build(self, depth, train=False, num_classes=20, random_init_fc8=False,
              debug=False):
        """
        Build the VGG model using loaded weights
        Parameters
        ----------
        depth: image batch tensor
            Depth Image. Scaled to ?
        train: bool
        ...
        with tf.name_scope('Processing'):
            ...
            if debug:
                depth = tf.Print(depth, [tf.shape(depth)],
                               message='Shape of input image: ',
                               summarize=4, first_n=1)
        self.conv1_1 = self._conv_layer(depth, "conv1_1")
    ...
    def get_conv_filter_random_init (self, name, shape):
    ...
    def get_conv_filter_1(self, name):
        temp = np.array (self.data_dict[name][0])
        temp = temp[:,:,0,np.newaxis,:]

        init = tf.constant_initializer(value=temp,
                                       dtype=tf.float32)
    ...
    def get_bias_random_init (self, name, num_classes=None):
