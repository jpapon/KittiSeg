...
def maybe_download_and_extract(hypes):
    ...
    image_data_dir = os.path.join(data_dir, 'data_RCTA/')

    if os.path.exists(vgg_weights) and os.path.exists(image_data_dir):
        return
    ...
    logging.info("Preparing MSTT data.")
...
def inputs(hypes, q, phase):
    ...
    tf.summary.image(tensor_name + '/tube0_image', tube0)
    rocks = tf.expand_dims(tf.to_float(label[:, :, :, 1]), 3)
    tf.summary.image(tensor_name + '/rocks', rocks)
    return image, label
