...
def preprocess_depth_image (image,gt_image):
...
def maybe_download_and_extract(hypes):
    """ Downloads, extracts VGG weights
...
def _load_gt_file(hypes, data_file=None):
    ...
    for epoche in itertools.count():
        ...
        for file in files:
            ...
            image = scipy.misc.imread(image_file, mode='F')
...
def jitter_input(hypes, image, gt_image):
    ...
    assert(image.shape[0:2] == gt_image.shape[0:2])
    return image, gt_image
